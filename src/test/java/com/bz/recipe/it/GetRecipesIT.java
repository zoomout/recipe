 ...
 * servings count) so assertions are independent of recipes created by other
 * tests.
 */
class GetRecipesIT extends BaseIntegrationTest {
    ...
    void filterByVegetarian_returnsOnlyVegetarianRecipes() {
        ...
        var ids = getRecipeIds("vegetarian=true&size=100");
        assertThat(ids)
    ...
    void filterByNonVegetarian_returnsOnlyRecipesWithNonVegetarianIngredients() {
        var ids = getRecipeIds("vegetarian=false&servings=" + markerServings + "&size=100");
    ...
    void filterByServings_returnsOnlyMatchingRecipes() {
        var ids = getRecipeIds("servings=" + markerServings + "&size=100");
    ...
    @Test
    void filterByExcludedIngredient_isCaseInsensitive_andRemovesRecipesContainingIt() {
        var ids = getRecipeIds(
    ...
    @Test
    void getRecipes_withoutFilters_returnsPagedResults() {
        givenReader()
            .when()
     ...
     */
    @Test
    void getRecipes_isSortedByNameAscendingByDefault() {
        var ids = getRecipeIds("servings=" + markerServings + "&size=100");
    ...
    @Test
    void getRecipes_withUnknownSortProperty_returns400() {
        givenReader()
            .when()
